...
class TLU(nn.Module):   #截断
  ...
  def forward(self, input):
    output = torch.clamp(input, min=-self.threshold, max=self.threshold)  #截断，限制到[min,max]
...
#        all_hpf_list_5x5 = np.array([[[[0,-1,0], [-1,4,-1], [0,-1,0]]]*30])
#        hpf_weight = nn.Parameter(torch.Tensor(all_hpf_list_5x5), requires_grad=False)
...
class YedNet(torch.nn.Module):
    def __init__(self):
        ...
        self.group1 = HPF()  # pre-processing Layer 1  滤波器预处理
        # self.conv0.weight = torch.nn.Parameter(srm)

        self.conv1 = torch.nn.Conv2d(in_channels=30, out_channels=30, kernel_size=5, stride=1,  #conv 30@(5 5 30)  stride:1 pad:2
        ...
        # Trunc T = 1
        self.tlu1 = TLU(1.0)
        self.pool = torch.nn.AvgPool2d(kernel_size=5, stride=2, padding=2)  # the same pool layer well be used to L3 and L4
